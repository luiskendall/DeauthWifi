...
# subprocess module to run commands on Kali Linux.
import subprocess
# regular expressions.
import re
# CSV files generated by airmon-ng, 
# we'll use the built-in csv module.
import csv
# os because we want to check for sudo
import os
#use time.sleep()
import time
# move .csv files in the folder if we found any. 
...
print(r"""
  ____   ____  ____   ____  ___      ___    ___  ____  
 /    | /    ||    \ |    ||   \    /  _]  /  _]|    \ 
|  o  ||  o  ||  o  ) |  | |    \  /  [_  /  [_ |  _  |
|     ||     ||     | |  | |  D  ||    _]|    _]|  |  |
|  _  ||  _  ||  O  | |  | |     ||   [_ |   [_ |  |  |
|  |  ||  |  ||     | |  | |     ||     ||     ||  |  |
|__|__||__|__||_____||____||_____||_____||_____||__|__|
...
# Instead of killing all processes, we just configure the chosen interface
subprocess.run(["sudo", "ifconfig", hacknic, "down"])
subprocess.run(["sudo", "iwconfig", hacknic, "mode", "monitor"])
subprocess.run(["sudo", "ifconfig", hacknic, "up"])
...
# print("Attack completed. Restoring NetworkManager...")
